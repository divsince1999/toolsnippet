...
export default function TabToSpaceTool() {
  const { input, setInput, output, setOutput, clearAll } = useTool();
  const [spaces, setSpaces] = useState(2);

  const handleConvert = () => {
    if (!input) return;
    const result = input.replace(/\t/g, " ".repeat(spaces));
    setOutput(result);
  };

  return (
    <ToolContainer title="Tab to Space Converter" description="Convert all tab characters to a consistent number of spaces.">
      <div className="grid gap-6">
        ...
        <div className="flex gap-2">
          <Button onClick={handleConvert}>Convert to Spaces</Button>
