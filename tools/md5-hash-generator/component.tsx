...
function md5(string: string): string {
  ...
  const utf8 = Array.from(enc.encode(string)).map((c) => String.fromCharCode(c)).join("");
...
export default function Md5HashGeneratorTool() {
  ...
  const generateMd5 = () => {
    try {
      ...
      setError(err instanceof Error ? err.message : "Failed to calculate MD5 hash.");
    }
  };
  ...
  return (
    <ToolContainer
      title="MD5 Hash & Checksum Generator"
      description="Compute standard 128-bit (32 hex characters) MD5 cryptographic checksums instantly."
    >
      <div className="grid gap-6">
        <TextArea
          ...
          placeholder="Enter text to generate MD5 checksum..."
          rows={5}
          error={error}
        />
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex gap-2">
            <Button onClick={generateMd5}>Generate MD5 Checksum</Button>
        ...
        {output && (
          <TextArea
            label="MD5 Hash (32 Hex Characters)"
            readOnly
            copyable
            value={output}
            rows={2}
